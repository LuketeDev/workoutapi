...
async def post(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Já existe uma categoria com o nome: {categoria_out.nome}",
        )
...
@router.get(
    "/categorias/{nome}",
    summary="Consultar dados de categoria pelo nome",
    status_code=status.HTTP_200_OK,
    response_model=List[CategoriaOut],
)
async def query(db_session: DatabaseDependency, nome: str) -> List[CategoriaOut]:
    query = select(CategoriaModel)

    if nome:
        query = query.where(CategoriaModel.nome.ilike(f"%{nome}%"))
